function Enum-ProcessMemoryWMI{
<#
.SYNOPSIS
...
#>
    Param(
        [Parameter()]
        [string]$ConfigFileName = "$dirConfigPath" + 'Enum-ProcessWMI.ini',
        [Parameter()]
		[switch]$CSV, 
    ...
    PROCESS{
        ...
        try{
            ...
            #$i=1
            $process = Get-WmiObject win32_process  -ComputerName $ComputerArray |
                        select Name, ProcessId, ParentProcessId, CommandLine, CreationDate, SessionId, @{l='User';e={($_.getowner().user)}}, @{l='Domain';e={($_.getowner().domain)}}, ExecutablePath, CSName
