function Enum-NetworkStatisticsWMI {
    <#
    .SYNOPSIS
		...
		#Loop through each line of results    
			foreach($result in $results) {
				...
				<#
					...
					#>
					#append the pso object to the resultsobject
					$resultsObject += $pso
					#Increment the progress counter
						$count++ 
				}#end if
			}# end foreach result block
		}#End ForEach Computer block
		if ($CSV){
			outcustom -object $resultsObject -CSV
		}else{
			outcustom -object $resultsObject
		}
	}#end process block
}
