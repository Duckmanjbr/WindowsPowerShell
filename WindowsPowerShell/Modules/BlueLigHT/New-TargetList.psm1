...
Specify an IPv4 network, requires the NetMask parameter.
...
.PARAMETER NoDeployList
Specify the path to a list of IPv4 addresses that should never have WinRM enabled.
...
.EXAMPLE
The following example builds a list of IP addresses from 10.10.10.1-10.10.10.254 and returns a list of hostnames that repsond to ping requests, have DNS entries, and are not included in a no-strike or no-deploy list.

PS C:\> New-TargetList -Cidr 10.10.10.0/24 -FindAlives -ResolveIp -NoStrikeList C:\pathto\NoStrikeList.txt -NoDeployList C:\pathto\NoDeployList.txt
...
[CmdletBinding()]
    ...
    ######### Remove Assets #########
    if ($PSBoundParameters['NoStrikeList'] -or $PSBoundParameters['NoDeployList']) {
        ...
        if ($PSBoundParameters['NoStrikeList']) {
            ...
            foreach ($Ip in $NoStrike) {
                if ( $Ip -match $IPv4 ) { $ExclusionList.Add($Ip) }
                    ...
                    try { $ResolvedIp = ([Net.DNS]::GetHostByName("$Ip")).AddressList[0].IPAddressToString }
                    catch { 
                        Write-Warning "$Ip does not resolve to a valid IPv4 address! Check no strike list formatting." 
                        continue
                    }
        ...
        elseif ($PSBoundParameters['NoDeployList']) {
            $NoDeploy = Get-Content $NoDeployList | Where-Object {$_ -notmatch "^#"}
            foreach ($Ip in $NoDeploy) {
                if ( $Ip -match $IPv4 ) { $ExclusionList.Add($Ip) }
                else { 
                    try { $ResolvedIp = ([Net.DNS]::GetHostByName("$Ip")).AddressList[0].IPAddressToString }
                    catch { 
    ...
    if ($FindAlives.IsPresent -and $ResolveIp.IsPresent) {
        ...
        foreach ($Address in $ValidTargets) {
            [void]$Pings.Add((New-Object Net.NetworkInformation.Ping).SendPingAsync(([Net.IPAddress]::Parse($Address)), 250))
        }
