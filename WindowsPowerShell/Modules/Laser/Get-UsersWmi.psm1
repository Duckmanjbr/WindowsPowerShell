...
[CmdLetBinding(SupportsShouldProcess = $false)]
    ...
    $RunspaceScript = { 
        ...
        try { $GroupUsers = Get-WmiObject @Parameters }
        catch { 
            Write-Warning ("{0}: {1}" -f $Computer,$_.Exception.Message)
            break
        }

        $UserList = New-Object Collections.ArrayList

        foreach ($User in $GroupUsers) {
            
            $UserAccount = [wmi]$User.PartComponent
            $Group = [wmi]$User.GroupComponent

            switch ($UserAccount.AccountType) {
                  0x100 { $AccountType = 'Temporary Duplicate' }
                  0x200 { $AccountType = 'Normal' }
                  0x800 { $AccountType = 'InterDomain Trust' }
                 0x1000 { $AccountType = 'Workstation Trust' }
                 0x2000 { $AccountType = 'Server Trust' }
                default { $AccountType = $UserAccount.AccountType }
            }
            ...
            $Properties = @{
		        ...
		        'SID' = $UserAccount.SID
                'ComputerName' = $User.__SERVER
            }
            [void]$UserList.Add((New-Object -TypeName PSObject -Property $Properties))
        }
