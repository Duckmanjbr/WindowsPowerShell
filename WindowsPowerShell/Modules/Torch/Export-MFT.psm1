...
The object(s) output by this module should be used as input to Get-MFT to retrieve the exported MFT from a remote machine.
...
.EXAMPLE
The following example extracts the master file table from the system volume and writes it to TEMP.

PS C:\> Export-MFT -TargetList Server01

.EXAMPLE
The following example extracts the master file table from the F volume on Server01 and writes it to TEMP.

PS C:\> Export-MFT -TargetList Server01 -Volume F
...
[CmdLetBinding(SupportsShouldProcess = $false)]
    ...
    $ErrorFileHosts,$ErrorFileVerbose = Set-ErrorFiles -ModuleName Export-MFT       
    $Global:Error.Clear()
    ...
    $RemoteScriptBlock = {
        ...
        $OutputFilePath = $env:TEMP + "\$([System.IO.Path]::GetRandomFileName())"
    ...
    if ($PSBoundParameters['TargetList']) {
        ...
        Invoke-Command -ComputerName $TargetList -ScriptBlock $RemoteScriptBlock -ArgumentList @($Volume) -SessionOption (New-PSSessionOption -NoMachineProfile) -ThrottleLimit $ThrottleLimit |
        ForEach-Object { 
